...
router.post("/create-new-game", function(req, res) {
    ...
    if (foundGameIndex > -1) {
        res.status(500).json({ message: 'Sorry, game already exists here!'});
    } else {
        let newGameObj = {
            id: uuidv4(),
            game,
...
router.put("/update-game/:id", function (req, res) {
    ...
    if (game.length === 0 || description.length === 0) {
        res.status(500).json({ message: "fields cannot be empty" }); //you want to make sure that you tell the user if the area is empty bc it cannot be empty
    }
    const { id } = req.params;
